...
export function WatchOnlyPanel() {
  const { raw, setRaw, analysis, scripthashes } = useWatchOnly();
      ...
      <CardContent className="space-y-3 text-sm text-muted-foreground">
        <div className="space-y-2">
          ...
          <Textarea
            ...
            rows={6}
            placeholder="One item per line: xpub, addresses, or txids. Chronicle will classify this locally."
        ...
        <p className="text-[0.65rem] text-muted-foreground">
          Debug (local only): {scripthashes.length} derived scripthashes • {buckets.length} demo
          buckets • {overlapCount} overlaps with current demo bucket scripthashes.
        </p>
      </CardContent>
    </Card>
  );
