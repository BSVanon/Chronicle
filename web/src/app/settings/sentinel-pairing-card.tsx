  ...
  const handleSaveAndTest = React.useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (data.ok) {
          const version = typeof (data as any).version === "string" ? (data as any).version : "unknown";
          let providerSummary = "";
          const providerStatus = (data as any).providerStatus;
          if (providerStatus && typeof providerStatus === "object") {
            const entries = Object.entries(providerStatus as Record<string, unknown>)
              .filter(([, value]) => typeof value === "string")
              .map(([key, value]) => `${key}: ${String(value)}`);
            if (entries.length > 0) {
              providerSummary = ` Providers: ${entries.join(", ")}.`;
            }
          }

          setStatus(`Health OK (version ${version}).${providerSummary}`);
        } else {
          setStatus(
            (data as any).message
              ? `Sentinel reported unhealthy: ${(data as any).message}`
              : "Sentinel reported unhealthy.",
          );
        }
      } else {
