...
export function DerivedBucketAssignmentPanel() {
  ...
  React.useEffect(() => {
    setSelection(() => {
      ...
      for (const hash of derivedHashes) {
        const owningBucket = buckets.find((bucket) =>
          bucket.scripthashes.includes(hash),
        );
        next[hash] = owningBucket ? owningBucket.id : "";
      }
      return next;
    });
  }, [derivedHashes, buckets]);
  ...
  const handleApply = React.useCallback(() => {
    const updated = buckets.map((bucket) => {
      // Remove any derived hashes from this bucket; they will be reassigned
      // based on the current selection.
      const base = bucket.scripthashes.filter(
        (hash) => !derivedHashes.includes(hash),
      );
      ...
      return { ...bucket, scripthashes: Array.from(next) };
    });
    setBuckets(updated);
  }, [buckets, selection, setBuckets, derivedHashes]);
    ...
    <Card className="mt-2">
      <CardHeader className="flex flex-row items-center justify-between gap-2">
        <div className="space-y-1">
          <CardTitle>Bucket assignment</CardTitle>
