...
export function SentinelStreamDevPanel() {
  ...
  const uniqueScripthashCount = React.useMemo(() => {
  ...
  const handleStart = React.useCallback(async () => {
    if (!hasRealSentinel) {
      setStatus("Pair a Sentinel on Inputs (Sentinel pairing card) before opening a live stream.");
      return;
    }
      ...
      for (const req of requests) {
        const handle = client.openShardStream(
          ...
          (closeEvent) => {
            ...
            if (ev.code === 1000 && ev.reason === "no active shards") {
              setStatus("Sentinel closed stream: no active shards for this wallet.");
            } else {
              const code = typeof ev.code === "number" ? ev.code : "?";
              const reason = ev.reason && ev.reason.length > 0 ? `: ${ev.reason}` : "";
              setStatus(`Sentinel stream closed (code ${code}${reason}).`);
            }
          },
        );
  ...
  const handleDeleteWallet = React.useCallback(async () => {
    if (!hasRealSentinel || mode !== "online_shielded") {
      setStatus("Switch to online_shielded and pair a Sentinel on Inputs before dropping a wallet.");
      return;
    }

    setStatus("Requesting wallet drop on Sentinel...");
    try {
      const result = await client.deleteWallet(walletId);
      if (result.ok) {
        setStatus("Wallet drop requested. Active shards and UTXO state cleared for this wallet.");
      } else {
        setStatus(`Wallet drop failed: ${result.message ?? "unknown error"}`);
      }
    } catch (error) {
      if (error instanceof Error) {
        setStatus(`Wallet drop error: ${error.message}`);
      } else {
        setStatus("Unknown error during wallet drop.");
      }
    }
  ...
  }, [client, hasRealSentinel, mode, walletId, reset, setStreamSource]);

  const totals = engine.getTotals();

  const orderedRecentEvents = React.useMemo(
    ...
    <Card className="mt-2">
      <CardHeader className="flex flex-row items-center justify-between gap-2">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            Once you&apos;ve paired a Sentinel on Inputs and enabled online_shielded
            mode, this panel sends the current shard plan, opens a /v1/stream
            WebSocket, and feeds incoming UTXO deltas into the shared UTXO
            engine.
          </p>
        </div>
      ...
      <CardContent className="space-y-3 text-xs text-muted-foreground">
        ...
        <div className="space-y-1">
          ...
          <p className="text-[0.7rem] text-muted-foreground">
            Unique scripthashes across all buckets: <strong>{uniqueScripthashCount}</strong>
          </p>
        </div>
        <Button
          type="button"
        ...
        {recentEvents.length > 0 && (
          <div className="space-y-2">
            ...
            <div className="space-y-1 max-h-64 overflow-y-auto pr-1">
              {orderedRecentEvents.map((event, index) => {
