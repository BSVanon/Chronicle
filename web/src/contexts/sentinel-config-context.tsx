...
const DEFAULT_SENTINEL_BASE_URL = process.env.NEXT_PUBLIC_SENTINEL_BASE_URL;
...
export function SentinelConfigProvider({
  ...
  React.useEffect(() => {
    try {
      ...
      if (DEFAULT_SENTINEL_BASE_URL && typeof DEFAULT_SENTINEL_BASE_URL === "string") {
        setConfigState({ baseUrl: DEFAULT_SENTINEL_BASE_URL });
      }
    } catch {
